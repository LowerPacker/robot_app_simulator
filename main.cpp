...
#include "common_interface/msg/app_cmd.hpp"
#include "common_interface/msg/mcu_info.hpp"


rclcpp::Node::SharedPtr g_node = nullptr;
common_interface::msg::AppCmd g_app_cmd_msg;
...
void topic_mcuinfo_callback(const common_interface::msg::McuInfo::SharedPtr msg) {
  // RCLCPP_INFO(g_node->get_logger(), "subscribe mcu_info: %s, %s, %s, %s, %s, %d", 
...
int main(int argc, char * argv[])
{
  ...
  auto publisher = g_node->create_publisher<common_interface::msg::AppCmd>("app_cmd", 10);
  auto subscriber = g_node->create_subscription<common_interface::msg::McuInfo>("mcu_info", 10, topic_mcuinfo_callback);
  std::thread input_thread(inputThread);
 
  rclcpp::WallRate loop_rate(50);
  while (rclcpp::ok()) {
    publisher->publish(g_app_cmd_msg);
